...
| ✅ | **Switching which slot is active** — after 7 failed attempts across 2 sessions (the box seemed to only ever work on slot A), completing `EXTRUDE_PROCESS`'s full sequence (stages 6/7 plus the toolhead-side prime moves between them - see [`docs/PROTOCOL.md`](docs/PROTOCOL.md)) turned out to be the missing piece: our earlier "successes" only ever did the first half. **Confirmed live** switching to slot D — `GET_FILAMENT_SENSOR_STATE`'s CONNECTIONS bank reported the box genuinely connected to D, not A, for the first time. See [`docs/TOOLCHANGE_TEST_PLAN.md`](docs/TOOLCHANGE_TEST_PLAN.md) phase 2 for the full (long) debugging trail, including a real regression scare along the way |
