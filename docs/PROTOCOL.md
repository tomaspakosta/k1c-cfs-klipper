...
- The cutter turned out to be a non-issue for the extrude side: **it's not a CFS protocol command at all.** It's a pure toolhead-motion sequence — home X/Y, move to a configured pre-cut position, then to a configured cut position — implemented entirely as Klipper G-code macros, with no bytes sent to the CFS box. **Correction from an earlier version of this page:** we initially described the cut position as dragging filament across a stationary blade edge, based on external reference material for a different hardware revision. On our unit the mechanism is **lever-actuated** — hand-testing confirmed pressing a physical lever cleanly cuts the filament.
  ...
  **Update, 2026-08-16: it stopped being the right position.** After further hardware handling, `X150 Y225` started actuating a different mechanism entirely — a purge/wipe flap near the cutter, not the cutter itself (easy to confuse the two by feel/sound alone; we did, more than once, before sorting it out). Re-calibrated the same way (manual jog, watch the lever, read back coordinates) and confirmed the real cutter position had moved to **`X=36.0, Y=227.0`** (`Y` pinned at this printer's max travel). **Lesson: don't treat a calibrated cut position as permanent** — if cuts stop working (or start hitting the wrong thing) after any physical handling of the printer/upgrade kit, re-run this calibration rather than assuming the old coordinates still apply. If your unit also has a nearby purge flap, physically distinguish it from the cutter lever before trusting either one's coordinates.
