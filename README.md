...
| ⚠️ | **Switching which slot is active is currently unreliable** — `RETRUDE`/`EXTRUDE` are solid on a *single* slot, but repeatedly asking the box to switch to a different slot mid-session failed 7 different ways in testing across 2 sessions; the box appears to only ever work on slot A regardless of which slot later commands target. We eventually found (by examining Creality's own official firmware for our board) that `EXTRUDE_PROCESS` has two more stages and toolhead-side moves we'd never sent — added to `cfs_cli.py`/`klipper_extra/creality_cfs.py` 2026-08-15, **not yet tested live**. See [`docs/TOOLCHANGE_TEST_PLAN.md`](docs/TOOLCHANGE_TEST_PLAN.md) phase 2 for the full writeup |
