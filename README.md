...
| ⚠️ | **`RETRUDE` of filament fed deep into the toolhead extruder can jam there**, needing a manual idler-lever release — same source as the fix above found the real cause and fix: a "tip-forming" unload sequence (re-shape the filament tip before the real pull, then retract much further than we originally tried) is now implemented in `CFS_RETRUDE`/`retrude_with_tip_form()`, but **not yet confirmed live** — see [`docs/PROTOCOL.md`](docs/PROTOCOL.md)'s "RETRUDE — the tip-forming unload sequence" |
